...
export function initGameCanvas(socket: WebSocket, playerRole: "left" | "right") {
	console.log("🕹️ Iniciando o canvas do jogo");
...
function startListening(socket: WebSocket) {
	socket.onmessage = (event) => {
		const data = JSON.parse(event.data);
		console.log("📥 Update recebido do outro player:", data);
		if (data.type === "update") {
...
function setupControls(socket: WebSocket) {
	document.addEventListener("keydown", (e) => {
		console.log("⬆️ Tecla:", e.key);  // 👈 DEBUG
...
function drawGame() {
	...
	for (const p of players) {
		const x = (p.posiX / 100) * (canvas.width - paddleWidth);
		const y = (p.posiY / 100) * (canvas.height - paddleHeight);
...
// const SERVER_URL = "ws://127.0.0.1:5000/game-ws";
...
// 	hideMainMenuAndShowCanvas(); // já tens isto
// 	setupControls(socket);
// 	startDrawing(socket);

// 	drawGameMessage("Waiting for opponent...");
