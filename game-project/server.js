    ...
    try {
        ...
        if (!row) {
            ...
            await new Promise((resolve, reject) => {
                db_game.run("INSERT INTO users (user_id, user_name, user_set_dificulty, user_set_tableSize, user_set_sound) VALUES (?, ?, 'normal', 'medium', 1)", [userId, username], function (err) {
                    if (err) {
...
gamefast.post("/save-settings", async (request, reply) => {
    ...
    try {
        await new Promise((resolve, reject) => {
            db_game.run(`UPDATE users SET 
                    user_set_dificulty = ?, 
                    user_set_tableSize = ?, 
                    user_set_sound = ?
                 WHERE user_name = ?`, [difficulty, tableSize, sound, username], function (err) {
                if (err) {
                    reject(err);
                }
                else {
...
gamefast.post("/save-match", async (request, reply) => {
    ...
    db_game.run(`INSERT INTO games (game_mode, game_player1_id, game_player2_id, game_player1_score, game_player2_score, game_winner) 
         VALUES (?, ?, ?, ?, ?, ?)`, [gameMode, player1Id, player2Id, player1Score, player2Score, winnerId], async function (err) {
        if (err) {
