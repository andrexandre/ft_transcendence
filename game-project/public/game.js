export function startSingleClassic(username, settings) {
    ...
    function update() {
        ...
        if (upPressed && playerY > 0)
            playerY -= paddleSpeed;
        if (downPressed && playerY < gameCanvas.height - paddleHeight)
            playerY += paddleSpeed;
        // Ball moves
        ballX += ballSpeedX;
        ballY += ballSpeedY;
        if (ballY <= 10 || ballY >= gameCanvas.height - 10)
            ballSpeedY *= -1;
        if (ballX <= 10 && ballY > playerY && ballY < playerY + paddleHeight)
            ballSpeedX *= -1;
        if (ballX >= gameCanvas.width - 10 && ballY > aiY && ballY < aiY + paddleHeight)
            ballSpeedX *= -1;
        if (ballX < 0) {
            aiScore++;
            checkWinCondition();
        ...
        if (aiY < 0)
            aiY = 0;
        if (aiY > gameCanvas.height - 80)
            aiY = gameCanvas.height - 80;
    }
    ...
    async function saveMatchToDatabase(player1Id, player2Id, player1Score, player2Score, gameMode, winnerId) {
        try {
