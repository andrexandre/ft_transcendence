export function startFreeForAll(player1, player2) {
    ...
    function update() {
        ...
        if (ballX <= 20 && ballY > player1Y && ballY < player1Y + paddleHeight)
            ballSpeedX *= -1;
        if (ballX >= gameCanvas.width - 20 && ballY > player2Y && ballY < player2Y + paddleHeight)
            ballSpeedX *= -1;
        ...
        if (wPressed && player1Y > 0)
            player1Y -= 4;
        if (sPressed && player1Y < gameCanvas.height - paddleHeight)
            player1Y += 4;
        if (upPressed && player2Y > 0)
            player2Y -= 4;
        if (downPressed && player2Y < gameCanvas.height - paddleHeight)
            player2Y += 4;
